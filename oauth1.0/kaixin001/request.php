...
$req_req = OAuthRequest::from_consumer_and_token($consumer, NULL, "GET", $config['request_token_url'],$parameters);
$req_req->sign_request($config['sig_method'], $consumer, NULL);

$response = $curl->get($req_req->to_url());
parse_str($response->body, $arr);
$param_str = 'oauth_token=' . $arr['oauth_token'];
session_start();
$_SESSION['request_token_secret'] = $arr['oauth_token_secret'];
$authorize_request_url = $config['authorize_url']."?".$param_str;
